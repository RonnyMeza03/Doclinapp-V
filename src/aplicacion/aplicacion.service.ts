...
export class AplicacionService {
  ...
  async update(id: number, updateAplicacionDto: UpdateAplicacionDto) {
    ...
    const actualizarAplicacion = Object.assign(aplicacionEncontrada, updateAplicacionDto)
  ...
  async findPacientesPorAplicacion(aplicacionID: number){
    ...
    if (aplicacionPacientes.length == 0){
      return new HttpException("Esta Aplicacion no tiene pacientes guardados", HttpStatus.NOT_FOUND)
    }
  ...
  async findUsuariosAplicacion(aplicacionID: number){
    ...
    if (aplicacionUsuarios.length == 0){
      return new HttpException("Esta Aplicacion no tiene usuarios guardados", HttpStatus.NOT_FOUND)
    }
